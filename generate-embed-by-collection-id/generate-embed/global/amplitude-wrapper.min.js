(function () {
  ...
  const initTrackingInfo = () => {
    window.ShopsenseEmbeds.trackingData = {
      session_id: generateUUID(), // Unique Per Session
      embed_id: '{{EMBED_ID_PLACEHOLDER}}', // Static, Unique Per Embed
  ...
  const init = () => {
    ...
    window.addEventListener('message', (event) => {
      if (event && event.data && 'utm_source' in event.data) {
  ...
  const logEvent = (_n, t = {}) => {
    ...
    if (!events[key]) {
      ...
      if (window.ShopsenseEmbeds.env === 'production') {
        window.ShopsenseEmbeds.amplitude.logEvent(n, Object.assign({}, t, getEventData()));
      }
    }
  };
